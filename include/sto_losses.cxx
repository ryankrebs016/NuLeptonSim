// Class to do stochastic losses for charged lepton propagation. 
// Requires unpacking of lookup tables and position, energy,
//  and denesity initialization before particle loops.
// At each step the new particle state needs to be set before 
// getting next interaction length and energy loss.
...
{   
    ...
    {
        printf("Couldn't find muon and/or tau tables\n");
        exit(-1);
    }
...
double stochastic_lepton_prop::get_interaction_length()
{  

    double rand_lin = (double)rand() / ((double)RAND_MAX + 1.);

    if(rand_lin==1) printf("invalid log, rand lin = 1 --> log 0, lead to broken particle?");

    double random_exponential = -log(1.-rand_lin);
...
double stochastic_lepton_prop::get_sampled_energy()
{
    ...
    rand_index = pow(10, rand_index) - 1.;
    ...
    // shoudn't get here, but in case
    printf("failed to get sampled energy");
    exit(EXIT_FAILURE);
}

/*
Example to interface with the class
int main()
{
    srand(time(NULL));
