...
int main(int argc, char **argv)
{
  ...
  nameEnergies+="taus_from_NuTauSim_";
  nameEnergies+=to_string(log10((double)atof(argv[1])));
  nameEnergies+="_";
  nameEnergies+=argv[2];
  if(argc>8){
    nameEnergies+="_";
  ...
  if(Depth2>min_depth) cout << "Angle too steep to be measured by IceCube" << endl;

  // Earth's chord (km) at thetaRef (deg) (R0 is radius of Earth in cm)
  cout << "Chord length Bottom" << R0*cos(PI*(1.-refTheta/180.))+sqrt(pow(R0*cos(PI*(1.-refTheta/180.)), 2.0)-(2*R0*Depth1-pow(Depth1,2.0)))*1.e-5 << "km" << endl;
  cout << "Chord length Top" << R0*cos(PI*(1.-refTheta/180.))+sqrt(pow(R0*cos(PI*(1.-refTheta/180.)), 2.0)-(2*R0*Depth2-pow(Depth2,2.0)))*1.e-5 << " km" << endl;
  ...
  for(Ldist=0.;!((Lmax1<=Ldist) && (Ldist<=Lmax2));)
    {
    //cout << Ldist << endl;
    // Get the local density for this part of the chord.
    dens = earthdens(&Ldist,&Lmax2);
    ...
    if(tag==0)
      ...
      if( traversed_grammage/d_grammage + 1. > 1000000.){
        Ldist = Lmax2; // NOTE: 1000000. is the size of the look-up table.
    ...
    if(tag==1)
      ...
      if(Ldist+dL > Lmax1) dL=Lmax1-Ldist;
