...
def load_LUT(LUT_fnm):
    ...
    P_exit = np.zeros(np.size(th_array))
...
if(plot_type =='energy'):
    colors = cm.hot(np.linspace(0, 1, int(2.*float(np.size(energy_list)))))
    ...
    for energy in energy_list[::-1]:
        th_em_array, P_exit, data_array, mean_num_CC, mean_num_NC, mean_num_decays = load_LUT('LUTs/%skm_ice_midCS_stdEL/LUT_%s_eV.npz'%(ice_thickness,energy))
        #print P_exit
        figure(1)
        ax=subplot(111)
        ax.set_xscale('log')
        ...
        if '3' not in energy and float(energy)>3.e16:
            ...
            img = np.zeros((51,np.size(arange(14.,21.1,0.1))-1))
            ...
            img = np.zeros((np.size(index_array),np.size(arange(14.,21.1,0.1))-1))
            #print img.shape
            for k in range(0,np.size(index_array)):
                #print th_em_array[k]
            ...
            #ax4.set_yscale('log')
            ax4.set_xscale('log')
            mu = np.zeros(np.size(th_em_array))
            eL68 = np.zeros(np.size(th_em_array))
            eH68 = np.zeros(np.size(th_em_array))
            eL95 = np.zeros(np.size(th_em_array))
            eH95 = np.zeros(np.size(th_em_array))
            eL99 = np.zeros(np.size(th_em_array))
            eH99 = np.zeros(np.size(th_em_array))
            for k in range(0,np.size(index_array)):
...
if(plot_type =='thickness'):
    colors = cm.hot(np.linspace(0, 1, int(2.*float(np.size(ice_thickness_list)))))
...
if(plot_type =='models'):
    ...
    for sigma_model in ['upp', 'mid','low']:
        for e_loss_model in ['low','std']:
            figure(100)
            th_em_array, P_exit, data_array, mean_num_CC, mean_num_NC, mean_num_decays  = load_LUT('LUTs/%skm_ice_%sCS_%sEL/LUT_%s_eV.npz'%(ice_thickness, sigma_model, e_loss_model,energy))
...
if(plot_type =='regen'):
    ...
    for energy in ['1e+20', '1e+19', '1e+18']:
        for regen_model in ['', '_no_regen']:
            figure(100)
            th_em_array, P_exit, data_array, mean_num_CC, mean_num_NC, mean_num_decays = load_LUT('LUTs/%skm_ice_%sCS_%sEL%s/LUT_%s_eV.npz'%(ice_thickness, sigma_model, e_loss_model, regen_model, energy))
